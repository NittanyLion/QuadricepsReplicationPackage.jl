...
printed), `moller` (Möller's bound), `prev` (the best count in the literature, `nothing` where
only the product grid exists), `src` (the code of the source of `prev`, starred when the
paper's rule was warm-started from theirs), and `shade` (`""`, `"light"` or `"dark"`).
"""
struct PaperRow
    family::String
    d::Int
...
function paper_rows(fam::AbstractString; dir::AbstractString = datadir())
    ...
    for l in eachline(joinpath(dir, "paper", "best_known_$fam.tex"))
        md = match(r"^\\multicolumn\{7\}\{@\{\}l\}\{\$d = (\d+)\$", l)
        md === nothing || (d = parse(Int, md[1]); continue)
        m = match(r"^(\\rowcolor\{gray!(\d+)\})?\s*(\d+) & (\d+) & ([\d.]+) & (\S+) & (\S+) & (\S+) & (.*?) \\\\\s*$", l)
        m === nothing && continue
        shade = m[2] === nothing ? "" : m[2] == "40" ? "dark" : "light"
        num(s) = s == "---" ? nothing : parse(Int, s)
        p = parse(Int, m[3])
        out[(d, p)] = PaperRow(fam, d, p, parse(Int, m[4]), m[5], m[6], num(m[7]), num(m[8]), String(m[9]), shade)
    end
    out
end
...
paper_row_tex(r::PaperRow) =
    string(SHADE[r.shade], r.p, " & ", r.N, " & ", r.rho, " & ", r.err, " & ",
           r.moller === nothing ? "---" : r.moller, " & ", r.prev === nothing ? "---" : r.prev, " & ", r.src, " \\\\")
